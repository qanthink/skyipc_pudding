...
MI_S32 Venc::createChn(MI_VENC_CHN vencChn, MI_VENC_ChnAttr_t *pstChnAttr)
{
	...
	if(NULL == pstChnAttr)
	{
		...
		//stChnAttr.stVeAttr.stAttrH265e.u32Profile = 0;	// 0 = main profile
		...
		stChnAttr.stRcAttr.stAttrH265Cbr.u32BitRate = 0.5 * 1024 * 1024;	// N * 1024 * 1024	 = N Mbps
		//stChnAttr.stRcAttr.stAttrH265Cbr.u32BitRate = 0.6 * 1024 * 1024;	// N * 1024 * 1024	 = N Mbps
		stChnAttr.stRcAttr.stAttrH265Cbr.u32SrcFrmRateNum = 60;
		stChnAttr.stRcAttr.stAttrH265Cbr.u32SrcFrmRateDen = 1;
		stChnAttr.stRcAttr.stAttrH265Cbr.u32Gop = 120;
...
MI_S32 Venc::createMainStream(MI_VENC_CHN vencChn, MI_VENC_ChnAttr_t *pstChnAttr)
{
	...
	if(NULL == pstChnAttr)
	{
		...
		/*set h265 chnnel rate control attribute*/
		stChnAttr.stRcAttr.eRcMode = E_MI_VENC_RC_MODE_H265CBR;
		stChnAttr.stRcAttr.stAttrH265Cbr.u32BitRate = 4 * 1024 * 1024;	// N * 1024 * 1024	 = N Mbps
...
-----------------------------------------------------------------------------*/
int Venc::rcvStream(MI_VENC_CHN vencChn, stStreamPack_t *pstStreamPack)
{
	//cout << "Call Venc::rcvStream()." << endl;

	// 入参检测
	if(NULL == pstStreamPack)
	{
	...
	(0 != stStream.u32Seq) ? (printf("stStream.u32Seq = %d", stStream.u32Seq)) : 0;
	...
	for(i = 0; i < stStream.u32PackCount; ++i)
	{
		...
		#endif
		if(stStream.pstPack[i].u32Len > u32PackMaxLen)
		{
			cerr << "stStream.pstPack[i].u32Len is out of range." << endl;
		}
		
		memset(pstStreamPack, 0, sizeof(stStreamPack_t));
		memcpy(pstStreamPack->u8Pack, stStream.pstPack[i].pu8Addr, stStream.pstPack[i].u32Len);
		pstStreamPack->u32Len = stStream.pstPack[i].u32Len;
		pstStreamPack->u64PTS = stStream.pstPack[i].u64PTS;

	}
	
	// step7: 释放流。ispahanV20中调整为，先调用SDK接口MI_VENC_ReleaseStream, 再释放用户空间free(stStream.pstPack)
...
注--意：
-----------------------------------------------------------------------------*/
void Venc::printVencStreamInfo(const MI_VENC_Stream_t* pstVencStream)
{
	...
	cout << "u32Len = " << setw(5) << pstVencStream->pstPack[0].u32Len << ", ";
	cout << "u64PTS = " << pstVencStream->pstPack[0].u64PTS << ", ";
	cout << "u32Offset = " << pstVencStream->pstPack[0].u32Offset << ", ";
	cout << "u32DataNum = " << pstVencStream->pstPack[0].u32DataNum << ", ";
	cout << "u32PackCount = " << pstVencStream->u32PackCount << ", ";
	cout << "u32Seq = " << pstVencStream->u32Seq << endl;
}
