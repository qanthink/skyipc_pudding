...
{
    //---[Enable AF]
    MS_S32 AF_DEBUG_ENABLE_AF;          /**< Enable/Disable AF */
    MS_S32 AF_DEBUG_LEVEL;              /**< 0: disable */
...
typedef struct
{
    ISP_AF_STATS stParaAPI[16];
} IQ_ISP_AF_INFO_STATS;

//roi
typedef struct
{
    ISP_AF_ROI_MODE mode;
    MI_U32 u32_vertical_block_number;
} IQ_CusAFRoiMode_t;

//win
typedef struct IQ_AF_WINDOW_PARAM_s
{
    MS_U32 u32StartX;                                           /*range : 0~1023*/
    MS_U32 u32StartY;                                           /*range : 0~1023*/
    MS_U32 u32EndX;                                             /*range : 0~1023*/
    MS_U32 u32EndY;                                             /*range : 0~1023*/
} IQ_AF_WINDOW_PARAM_t;

typedef struct
{
    MS_U8 u8WindowIndex;
    ISP_AF_WIN stParaAPI;
} IQ_CusAFWin_t;

typedef struct
{
    MS_U16 coeff[70];
} ISP_AF_FILTER_ATTR_GEO;

typedef struct
{
    MS_U32                MinMotorPos;     /** min motor position **/        //default: 0
    MS_U32                MaxMotorPos;     /** max motor position **/        //default: 1023
    MS_U32                MinMotorStep;    /** min motor moving step **/     //default: 1023
    MS_U32                MaxMotorStep;    /** max motor moving step **/     //default: 1023
    MS_U32                InitPos;         /** initial motor position **/    //default: 0
} ISPAFInitParam_t, AfInitParam_t;
...
typedef struct
{
    ...
    MS_U32                fps;             /** fps **/
    IQ_ISP_AF_INFO_STATS  *af_stats;       /** AF statistic **/
    MS_U32                CurPos;          /** current position **/
    AFDEBUG_t             *tAFDebug;       /**< AF debug -> unit test*/
} ISPAFInfo_t, AfInput_t;

typedef struct
{
    MS_U32                  Change_Motor;    /** if true, apply the following 1 result to motor **/
    MS_U32                  FocusPos;        /** focus position **/

    MS_U32                  Change_WinInfo;  /** if true, apply the following 4 results to hw register **/
    IQ_CusAFRoiMode_t       af_roimode;      /** roi **/
    IQ_CusAFWin_t           af_win[16];      /** win **/
    ISP_AF_FILTER_ATTR_GEO  af_filter;       /** filter **/
    ISP_AF_FILTERSQ         af_filtersq;     /** square **/
}__attribute__((packed, aligned(4))) ISPAFResult_t, AfOutput_t;

MS_S32 DoAf(AfHandle Handle, AfInput_t *pInput, AfOutput_t *pOutput);
AfHandle AfInit(ISP_DBGBUF_HANDLE CfgHandle, AfInitParam_t *init_param, AfInput_t *pInput, AfOutput_t *pOutput);
MS_S32 AfRelease(AfHandle Handle);
